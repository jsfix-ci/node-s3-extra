...
const s3extra = (s3, queue) => {
  ...
  const uploadFile = async (file, s3Url, params = {}) => {
    ...
    const fileType = await FileType.fileTypeFromStream(fs.createReadStream(file));
    ...
    if (fs.lstatSync(contentPath).isDirectory()) {
      ...
      for (const fileName of files) {
        ...
        if (fs.lstatSync(filePath).isDirectory()) {
          promises.push(
            uploadFileOrFolder(filePath, `${s3Folder}${fileName}`, params)
          );
          continue;
        }

        promises.push(queue.add(() => uploadFile(filePath, s3Folder, params)));
      }
